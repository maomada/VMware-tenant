...
import { admin, getApiErrorMessage, type AdminProject, type AdminUser } from '../../api';

export default function AdminProjects() {
  const [data, setData] = useState<AdminProject[]>([]);
  const [users, setUsers] = useState<AdminUser[]>([]);
  const [loading, setLoading] = useState(false);
  const [editModal, setEditModal] = useState<AdminProject | null>(null);
  ...
  const openEdit = (record: AdminProject) => {
    setEditModal(record);
  ...
  const columns = [
    ...
    {
      title: '操作', render: (_: unknown, record: AdminProject) => (
